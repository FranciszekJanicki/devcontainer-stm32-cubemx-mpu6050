...
{
    return -(std::atan2(accel_scaled.x, std::sqrt(accel_scaled.y * accel_scaled.y + accel_scaled.z * accel_scaled.z)) *
             180.0F) /
           3.1416F;
}
...
void MPU6050::initialize_advanced(std::uint32_t const sampling_rate, DLPF const dlpf, DHPF const dhpf) const noexcept
{
...
{
    this->i2c_device_.write_bit(std::to_underlying(RA::INT_ENABLE), enabled, std::to_underlying(INT_STATUS::ZMOT_BIT));
}

void MPU6050::set_int_motion_enabled(bool const enabled) const noexcept
{
    this->i2c_device_.write_bit(std::to_underlying(RA::INT_ENABLE), enabled, std::to_underlying(INT_STATUS::MOT_BIT));
}

void MPU6050::set_int_free_fall_enabled(bool const enabled) const noexcept
{
    this->i2c_device_.write_bit(std::to_underlying(RA::INT_ENABLE), enabled, std::to_underlying(INT_STATUS::FF_BIT));
}
...
{
    return this->i2c_device_.read_bit(std::to_underlying(RA::INT_STATUS), std::to_underlying(INT_STATUS::DATA_RDY_BIT));
}
...
{
    this->i2c_device_.write_bit(std::to_underlying(RA::USER_CTRL), enabled, std::to_underlying(USER_CTRL::FIFO_EN_BIT));
}

void MPU6050::reset_fifo() const noexcept
{
    this->i2c_device_.write_bit(std::to_underlying(RA::USER_CTRL), true, std::to_underlying(USER_CTRL::FIFO_RESET_BIT));
}
...
{
    this->i2c_device_.write_bit(std::to_underlying(RA::PWR_MGMT_1), enabled, std::to_underlying(PWR_MGMT_1::SLEEP_BIT));
}

void MPU6050::set_wake_cycle_enabled(bool const enabled) const noexcept
{
    this->i2c_device_.write_bit(std::to_underlying(RA::PWR_MGMT_1), enabled, std::to_underlying(PWR_MGMT_1::CYCLE_BIT));
}
